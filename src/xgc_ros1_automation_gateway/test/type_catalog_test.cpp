...
TEST(MavrosCatalog, InstantiatesAndSerializesEveryInstalledMessage) {
  const std::string package_path = ros::package::getPath("mavros_msgs");
  ASSERT_FALSE(package_path.empty());
  const auto messages = catalogTypes(package_path + "/msg", ".msg");
  ASSERT_GE(messages.size(), 60U);

  TypeRegistry types;
  JsonCodec codec;
  for (const auto& name : messages) {
    SCOPED_TRACE(name);
    const std::string type = "mavros_msgs/" + name;
...
TEST(MavrosCatalog, InstantiatesAndSerializesEveryInstalledServiceSide) {
  const std::string package_path = ros::package::getPath("mavros_msgs");
  ASSERT_FALSE(package_path.empty());
  const auto services = catalogTypes(package_path + "/srv", ".srv");
  ASSERT_GE(services.size(), 30U);
...
int main(int argc, char** argv) {
  ros::init(
      argc, argv, "xgc_ros1_automation_gateway_unit_tests",
